...
-(void)replaceWithPath:(NSString *)path {
    ...
    printf("replaceWithPath filelist = %s\n", [[NSString stringWithFormat:@"%@", filelist] UTF8String]);
        ...
        if (exists) {
            if (isDir) {
                ...
                if (![newFileContent isEqualToString:fileContent]) {
                    printf("replaceWithPath fullPath = %s\n", [[NSString stringWithFormat:@"%@", fullPath] UTF8String]);
                    NSArray *sepeArray = [fullPath componentsSeparatedByString:@"podTest3/"];
